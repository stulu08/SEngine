...
namespace Stulu {
    ...
    inline fini_string_t& l_trim(fini_string_t& str, const fini_string_t trim_chars = "\t\v\f; ") {
        str.erase(0, str.find_first_not_of(trim_chars)); return str;
    }

    inline fini_string_t& r_trim(fini_string_t& str, const fini_string_t trim_chars = "\t\v\f; ") {
        str.erase(str.find_last_not_of(trim_chars) + 1); return str;
    }

    inline fini_string_t& trim(fini_string_t& str, const fini_string_t trim_chars = "\t\v\f; ") {
    ...
    public:
        /// Define
        static inline int PARSE_FLAGS = 0, SAVE_FLAGS = 0;
        ...
        enum parse_e { PARSE_COMMENTS_SLASH = 1 << 0, PARSE_COMMENTS_HASH = 1 << 1, PARSE_COMMENTS_SEMICOLON = 1 << 2, PARSE_COMMENTS_ALL = 1 << 3 };
    ...
    public:
        ...
        /// Methods
        INI(const INI& from) : source(from.source), filename(from.filename) {
            // Deep clone INI
        ...
        INI(INI&& from) noexcept
            : source(from.source), filename(from.filename) {
            std::swap(this->sections, from.sections);
            std::swap(this->current, from.current);
        }
        INI(fini_string_t filename, bool doParse, int parseFlags = 0) : source(SOURCE_FILE), filename(filename) {
            this->create("");
        ...
        INI& operator=(const INI& other) {
            if (this != &other) {
                ...
                if(sections.size() > 0)
                    this->current = sections.begin()->second;
            }
        ...
        void _parseFile(fini_ifstream_t& file, int parseFlags) {
            ...
            while (std::getline(file, line)) {
                ...
                if (parseFlags & PARSE_COMMENTS_HASH || parseFlags & PARSE_COMMENTS_ALL)
                    line = line.substr(0, line.find("#"));
                if (parseFlags & PARSE_COMMENTS_SEMICOLON || parseFlags & PARSE_COMMENTS_ALL)
                    line = line.substr(0, line.find(";"));
                ...
                if ('[' == line.at(0)) { // Section
                    ...
                    if (sections.find(section) != sections.end()) {
                        CORE_ERROR("cpp-feather-ini-parser: Duplicate section '{0}': {1}", section, i);
                        throw - 1;
                    }
                    ...
                    if (indexEquals != fini_string_t::npos) {
                        ...
                        if ((*current).find(key) != (*current).end()) {
                            CORE_ERROR("cpp-feather-ini-parser: Duplicate key '{0}': {1}", key, i);
                            throw - 1;
                        }
        ...
        void create(fini_string_t section) {
            if (section != "" && sections.find(section) != sections.end()) {
                CORE_ERROR("cpp-feather-ini-parser: Duplicate section '{0}'", section);
                throw - 1;
            }
        ...
        template<class T>
        void setAs(fini_string_t section, fini_string_t key, T value) {
            fini_sstream_t stream;
            stream << value;
            set(section, key, stream.str());
        }
        template<class T>
        void setAs(fini_string_t key, T value) {
            fini_sstream_t stream;
            stream << value;
            set(key, stream.str());
        }
