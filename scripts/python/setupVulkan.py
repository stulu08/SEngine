...
class VulkanConfiguration:
    ...
    latesVulkanVersion = "1.3.290.0"
    vulkanDirectory = "./Dependencies/VulkanSDK"
    ...
    def __InstallVulkanSDK(cls):
        ...
        while not permissionGranted:
            reply = str(input("Would you like to install VulkanSDK {0:s}? [Y/N]: ".format(cls.latesVulkanVersion))).lower().strip()[:1]
            if reply == 'n':
                return
            permissionGranted = (reply == 'y')

        vulkanInstallURL = f"https://sdk.lunarg.com/sdk/download/{cls.latesVulkanVersion}/windows/VulkanSDK-{cls.latesVulkanVersion}-Installer.exe"
        vulkanPath = f"{cls.vulkanDirectory}/VulkanSDK-{cls.latesVulkanVersion}-Installer.exe"
