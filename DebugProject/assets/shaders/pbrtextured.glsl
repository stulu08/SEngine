...
void main()
{
   u_mat.albedo = pow(texture(albedoMap, vertex.texCoords).rgb, vec3(2.2));
   u_mat.metallic = texture(metallicMap, vertex.texCoords).r;
   u_mat.roughness = texture(roughnessMap, vertex.texCoords).r;
   u_mat.ao = texture(aoMap, vertex.texCoords).r;
    

   vec3 cameraPos = vec3(cameraPosition);
   vec3 cameraRot = vec3(cameraRotation);
   vec3 N = getNormalFromMap(vertex.worldPos, vertex.texCoords, vertex.normal, normalMap);
